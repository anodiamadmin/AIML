...
import argparse
from frac import Full3Net, Full4Net, DenseNet
...
def graph_output(net):
    xrange = torch.arange(start=-18, end=18.01, step=0.02, dtype=torch.float32)
    yrange = torch.arange(start= -4, end=26.01, step=0.02, dtype=torch.float32)
    xcoord = xrange.repeat(yrange.size()[0])
    ycoord = torch.repeat_interleave(yrange, xrange.size()[0], dim=0)
    grid = torch.cat((xcoord.unsqueeze(1),ycoord.unsqueeze(1)),1)
...
parser.add_argument('--epoch', type=int,default='100000', help='max training epochs')
...
if list(net.parameters()):
    ...
    plt.savefig('./plot/crown.png',format='png')
