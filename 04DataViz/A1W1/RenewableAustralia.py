...
def config_options():
    ...
    gdf.to_file("australian-states-polygons.geojson", driver="GeoJSON")
...
def draw_choropleth_animated(df_percent_renewable, df_renewable, df_nonrenewable):
    ...
    for year in years:
        ...
        fig.write_image(temp_path, width=1024, height=768)
        time.sleep(0.2)
        try:
    ...
    imageio.mimsave("RenewableAustralia.gif", images, fps=1)  # <- Updated here (GIF output)
    print(f"Saved GIF animation with {len(images)} frames.")
